  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      ...
      const [queueRes, inProgRes, completedRes] = await Promise.all([
        fetch('http://localhost:3000/api/reports?status=APPROVED_TO_DEPT&limit=50', {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch('http://localhost:3000/api/reports?status=IN_PROGRESS&limit=1', {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch('http://localhost:3000/api/reports?status=COMPLETED&limit=50', {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
      ...
      const completedToday = completed.data.filter(
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-slate-100 p-6 font-sans">
      ...
      {actionReportId && (
        <ActionPanel
          ...
          token={getToken()}
          onSuccess={() => { setActionReportId(null); fetchReports(); }}
